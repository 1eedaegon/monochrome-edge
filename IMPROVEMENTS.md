...
- [ ] ✅ **Stepper 좁은 폭 라벨 겹침 (2026-07-20 puppeteer 실측)**: snake 반응형·상태 이동(`updateStep`)·말줄임 "..." 부착은 정상 동작 확인. 문제는 `truncateText(text, 30)`(stepper.ts:142)가 **가용 폭과 무관하게 30자 고정 상한** → 320px에서 라벨이 인접 노드와 겹치고, 220px 세로 전환 시 `text-anchor:middle` 라벨이 컨테이너 좌우로 오버플로(클리핑 없음). text 타입은 세로 폴백이 없어 좁은 화면에서 심하게 겹침. → 노드 간격(gap) 비례 동적 문자수 or 라벨에 SVG `foreignObject`+CSS `text-overflow` 적용, text 타입도 ≤240px 세로 폴백 추가. (S-M) *(검증 상세: snake는 768→1행, 375→2행, 320→3행, ≤240→세로 전환 모두 정상)*

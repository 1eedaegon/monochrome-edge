...
export class TabComponent {
    ...
    createDOM() {
        ...
        this.menuButton.innerHTML = `
            <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                <path d="M3 5h10M3 8h10M3 11h10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
            </svg>
        `;
    ...
    updateScrollButtons() {
        ...
        if (needsScroll) {
            this.scrollLeftBtn.style.display = this.tabList.scrollLeft > 0 ? 'block' : 'none';
            this.scrollRightBtn.style.display =
                this.tabList.scrollLeft < (this.tabList.scrollWidth - this.tabList.clientWidth) ? 'block' : 'none';
        } else {
            this.scrollLeftBtn.style.display = 'none';
            this.scrollRightBtn.style.display = 'none';
        }
    ...
    showDropdownMenu(event) {
        ...
        if (this.tabs.length > 0) {
            ...
            const actions = [
                ...
                { label: 'Close Others', action: () => this.closeOtherTabs(this.activeTabId) }
            ];

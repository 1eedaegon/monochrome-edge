...
$.fn.mceIconToggle = function (options?: IconToggleOptions) {
  ...
  return this.each(function () {
    ...
    let state = readIconToggleState(settings.type!);

    const getIcons = () => {
      const [icon1, icon2] = iconToggleIcons(settings.type!);
      return { icon1, icon2 };
    };
    ...
    $btn.on("click", function () {
      ...
      state = nextIconToggleState(settings.type!, state);
      $btn.attr("data-state", state);
      applyIconToggleState(settings.type!, state);
